...
from radioactive_ralph.work_discovery import discover_all_repos, discover_work
...
async def test_discover_all_repos_success(tmp_path: Path) -> None:
    ...
    # Both empty, so both should generate both work items (missing tests, missing docs)
...
async def test_discover_all_repos_exception(tmp_path: Path, mocker) -> None:
    ...
    mocker.patch(
        "radioactive_ralph.work_discovery.discover_work",
        side_effect=[
