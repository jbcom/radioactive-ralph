...
- [x] post-release-audit workflow — DONE (55 agents, 7 lenses, adversarially verified): 29 confirmed findings (3 high, 13 medium, 13 low). Saved to scratchpad/audit-findings.json. Fixing all on branch chore/post-release-audit.
  - [ ] Fix 3 HIGH: (a) store/tasks.go MarkFailed UPDATEs lack `AND status='running'` guard → late worker resurrects/stomps a reclaimed+reassigned task (double-exec + dropped completion); (b) ipc/server.go acceptLoop `return`s on ANY transient Accept error → permanently wedges the control plane (comment says "log and continue"); (c) orch/orchestrator.go claimStepTask swallows ALL CreateTask errors (not just the benign UNIQUE dup) → silent permanent stall on a real insert failure.
  - [ ] Fix 13 MEDIUM (ipc read-deadline; agent readLoop back-pressure drop; Attach ctx-cancel goroutine leak; dead HeartbeatInterval; plan CLI zero behavioral tests; ProjectAbsPath ordering untested; 4 docs-accuracy drifts; plan-import "active" lies without supervisor; TUI empty-state dead end; discovery live-but-unresponsive untested).
  - [ ] Fix 13 LOW (dead branches; recorder mutex; /tmp socket perms note; fanout mid-loop wedge; retry-exhaustion coverage; more docs drift; doctor `run`/opencode drift; no-supervisor hint → service install; TUI newline/status-color polish).
  - [ ] Land build/test/-race/lint green; PR it.
