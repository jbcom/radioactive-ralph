...
- [ ] [WAIT-REVIEW] Real native installers/packaging — BUILT on feat/native-packaging, opening PR. Spec: docs/superpowers/specs/2026-07-17-native-packaging-design.md. DONE: goreleaser nfpms (.deb/.rpm, verified), winget publisher, GUI-bundle release.yml matrix (fyne package --tags gui per-OS → macOS .app+ad-hoc-codesign+.dmg+Homebrew cask PR, Linux AppImage via build-appimage.sh + .desktop, Windows .exe), FyneApp.toml + 512² icon, install.sh dead-model fix, packaging-lint CI job (goreleaser check + shellcheck + desktop-file-validate). macOS path (fyne package→valid .app→ad-hoc sign verifies) proven locally on darwin; Linux/Windows validated on first release tag. OSS-free signing: macOS cask needs no Apple acct; Windows SignPath is the one optional free-signup (add SIGNPATH_* secret → MSI signs, else ships unsigned). Blocked only on CI green → self squash-merge. Optional follow-up: Windows MSI via wix + SignPath enrollment when the user does the free signup.
