...
- [x] post-release-audit workflow — DONE (55 agents, 7 lenses, adversarially verified): 29 confirmed findings (3 high, 13 medium, 13 low). Saved to scratchpad/audit-findings.json. Fixing all on branch chore/post-release-audit.
  ...
  - [ ] [WAIT-REVIEW] Babysit PR #76 (all 29 audit fixes) to green squash-merge; monitor armed. Then a SECOND audit pass on the fixed tree (the fixes themselves are new code that deserves the same lens) before declaring the audit converged.
