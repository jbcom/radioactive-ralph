...
- [ ] GENERATED 2026-07-28 by a THIRD dogfooding pass, run against the merged
      main (the terminal-blocker PR had just landed). The
      ...
      So the work is small and bounded: render recent events (or at least the
      failure summary for a failed task) in the human path, reusing the
      existing FailureSummary rather than re-deriving a second taxonomy.
