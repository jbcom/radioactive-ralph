...
- [x] post-release-audit workflow — DONE (55 agents, 7 lenses, adversarially verified): 29 confirmed findings (3 high, 13 medium, 13 low). Saved to scratchpad/audit-findings.json. Fixing all on branch chore/post-release-audit.
  ...
  - [ ] [WAIT-REVIEW] Babysit PR #81 (5 third-pass fixes) to green squash-merge; monitor armed. Fourth pass on #81's fix code once merged. Audit trend: 29→14→5 confirmed — converging but not yet zero; keep iterating until a pass finds ~nothing. Then desktop-app + onboarding.
