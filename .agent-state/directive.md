...
- [x] post-release-audit workflow — DONE (55 agents, 7 lenses, adversarially verified): 29 confirmed findings (3 high, 13 medium, 13 low). Saved to scratchpad/audit-findings.json. Fixing all on branch chore/post-release-audit.
  ...
  - [ ] [WAIT-REVIEW] Babysit PR #79 (14 second-pass fixes) to green squash-merge; monitor armed. The second pass found real bugs, so run a THIRD pass on THIS PR's fix code once merged — stop only when a pass confirms ~zero (convergence). Then desktop-app + onboarding.
