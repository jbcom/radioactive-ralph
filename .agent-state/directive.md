...
- [ ] The agent CANNOT write the decision log, and telling it to is worse than
      ...
      AND THE GOAL ITSELF IS PARTLY WRONG. "Surface WHAT the provider asked
      for" means putting provider prose on an operator surface, which the
      content-safety boundary exists to prevent: only a CLOSED SET of fixed
      constants crosses (operator_snapshot.go:262-268), never text from an
      external process. BlockReason is documented "provider-output-free" for
      the same reason.
      So the useful version is NOT free text. Options that respect the
      boundary:
        - a closed taxonomy of prompt KINDS (permission / credential /
          clarification / destructive-op confirm), classified from the stream
          by Ralph the way failure categories already are;
        - a count or fingerprint of the prompt, so repeated identical blocks
          are distinguishable from varied ones, with no prose crossing.
      A containment write-path grant for the log directory remains possible for
      AGENT-authored notes, but that is a separate, weaker want -- and it puts
      arbitrary agent text one hop from an operator surface, so it needs the
      same boundary answered before it is worth building.
      What this thread actually established: Ralph's own classification works
      and is verified; the missing thing is a FINER TAXONOMY, not prose.
