...
- [ ] Fyne GUI client (tray/menubar + full window per later design decision), peer to the TUI on the same socket; shares store/ipc types directly (no serialization boundary to redefine). DESIGN DONE (docs/superpowers/specs/2026-07-17-fyne-gui-client-design.md, in PR #87): read+drive Controller seam extending the TUI's read-only DataSource; CONSISTENT identity (mirrors TUI semantic palette + macro/meso/micro drill); Fyne isolated behind a `gui` build tag (Fyne is CGO-only, would break CI's CGO-off six-way cross-build — verified by probe) with a dedicated CGO-on GL-headers CI job; the Fyne-free half (Controller iface + statusColor + anti-drift test) stays untagged. IMPLEMENT after #87 merges, off fresh main.
