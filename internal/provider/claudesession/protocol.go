// Package claudesession wraps a `claude -p` subprocess so the claude
// provider runner can drive it via stream-json over stdin/stdout.
//
// The stream-json protocol pairs message objects, one per JSON line:
//
//   - Messages FROM the runtime to Claude carry `{"type":"user", …}`
...
// so downstream consumers (event log, runtime) can re-emit or
// re-parse as needed.
type Inbound struct {
