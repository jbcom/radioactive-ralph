...
func Parse(md []byte) (*Plan, error) {
	...
	metadataCount, stepCount := countMetadata(groups)
	return &Plan{Groups: groups, V2: stepCount > 0 && metadataCount == stepCount}, nil
}
	...
	for n := node; n != sectionEnd && n != nil; n = n.NextSibling() {
		...
		case *ast.List:
			...
			for item := v.FirstChild(); item != nil; item = item.NextSibling() {
				...
				metadata, metadataErr := parseTaskMetadataBlock(item, source)
				if metadataErr != nil {
					return nil, false, metadataErr
				}
				steps = append(steps, Step{
					Text: text, RequiresApproval: requiresApproval, Metadata: metadata,
				})
			}
		case *ast.Paragraph:
