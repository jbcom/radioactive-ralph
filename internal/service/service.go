...
//   - Linux/WSL → systemd user unit (~/.config/systemd/user/ralph-<variant>.service)
...
//     passed the gate flag to `ralph service install` explicitly.
//
// Service-context detection at `ralph run` time uses:
//
...
type InstallOptions struct {
	...
	HomeDir string
	// RalphBin is the absolute path to the ralph binary that the unit
	// should exec. Required.
	RalphBin string
	...
	// Enforces "operator explicitly passed --confirm-X to ralph service
	// install" so gates aren't bypassed via the service wrapper.
	GateConfirmed bool
...
// Does not load it into launchd/systemd — callers do that via `ralph
// service start` to keep Install a pure filesystem operation (trivial
// to test and to undo).
