...
func TestUnitNameFormat(t *testing.T) {
	...
	if got := UnitName(BackendSystemdUser, "green"); got != "ralph-green" {
...
func TestUnitPathSystemd(t *testing.T) {
	...
	want := "/tmp/home/.config/systemd/user/ralph-green.service"
	if got != want {
		t.Errorf("UnitPath = %q, want %q", got, want)
	}
}
...
func TestInstallLaunchdWritesPlist(t *testing.T) {
	...
	for _, needle := range []string{
		...
		`<key>ProgramArguments</key>`, `/usr/local/bin/ralph`,
...
func TestInstallSystemdWritesUnit(t *testing.T) {
	...
	for _, needle := range []string{
		...
		"[Service]", "Type=simple",
		"ExecStart=/usr/local/bin/ralph run --variant green --foreground",
...
func TestLaunchdRendererEscapesXML(t *testing.T) {
	...
	out, err := Install(InstallOptions{
		Backend:  BackendLaunchd,
		HomeDir:  t.TempDir(),
		RalphBin: "/path/with <angle> & ampersand/ralph",
		Variant:  p,
	})
	if err != nil {
		t.Fatalf("Install: %v", err)
