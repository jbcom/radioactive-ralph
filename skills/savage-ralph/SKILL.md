...
This skill drives `savage-ralph` via the `radioactive_ralph` MCP server.
...
When the operator invokes `/savage-ralph`, walk through these steps:
...
2. `plan.next` with `variant: "savage"` to see what's ready
3. `variant.spawn` with `variant_name: "savage"` to launch a subprocess
