...
This skill drives `blue-ralph` via the `radioactive_ralph` MCP server.
...
When the operator invokes `/blue-ralph`, walk through these steps:
...
2. `plan.next` with `variant: "blue"` to see what's ready
3. `variant.spawn` with `variant_name: "blue"` to launch a subprocess
