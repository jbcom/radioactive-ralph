...
This skill drives `green-ralph` via the `radioactive_ralph` MCP server.
...
When the operator invokes `/green-ralph`, walk through these steps:
...
2. `plan.next` with `variant: "green"` to see what's ready
3. `variant.spawn` with `variant_name: "green"` to launch a subprocess
