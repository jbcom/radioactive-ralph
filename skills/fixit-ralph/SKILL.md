...
This skill drives `fixit-ralph` via the `radioactive_ralph` MCP server.
...
When the operator invokes `/fixit-ralph`, walk through these steps:
...
2. `plan.next` with `variant: "fixit"` to see what's ready
3. `variant.spawn` with `variant_name: "fixit"` to launch a subprocess
