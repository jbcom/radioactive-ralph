...
| Meso | One plan's tasks with per-task status and the same markers the TUI shows — `w:…<suffix>` for the holding worker, `via <alias>` once executed, `p1`/`p2` for a shared ready partition — plus a wrapping continuation row carrying a blocked task's remediation | Pause / Resume / Abandon the plan; Approve a task awaiting approval |
