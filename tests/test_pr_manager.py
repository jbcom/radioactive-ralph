...
async def test_scan_repo_success(mocker, mock_forge_client):
    mocker.patch(
        "radioactive_ralph.pr_manager.GitClient.get_remote_url",
...
async def test_scan_repo_exception(mocker):
    ...
    mocker.patch(
        "radioactive_ralph.pr_manager.get_forge_client", side_effect=Exception("API Error")
    )
...
async def test_scan_all_repos(mocker):
    mock_scan = mocker.patch(
        "radioactive_ralph.pr_manager.scan_repo",
        side_effect=[
