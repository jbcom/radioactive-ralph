...
def test_discover_github_token_errors(mocker):
    from radioactive_ralph.github_client import get_github_token, AuthError
...
async def test_list_prs(forge_info, mocker):
    ...
    with respx.mock:
        respx.get("https://api.github.com/repos/org/repo/pulls?state=open&per_page=100").respond(json=pr_data)
        async with forge as client:
...
async def test_get_pr_ci(forge_info, mocker):
    ...
    with respx.mock:
        # Test success case
        respx.get("https://api.github.com/repos/org/repo/commits/12345/check-runs?per_page=100").respond(
        ...
        respx.get("https://api.github.com/repos/org/repo/commits/12345/check-runs?per_page=100").respond(
            json={"check_runs": [{"name": "lint", "status": "completed", "conclusion": "unknown_conclusion"}]}
        )
        respx.get("https://api.github.com/repos/org/repo/commits/12345/status").respond(
            json={"statuses": []}
        )
...
async def test_get_pr_reviews(forge_info, mocker):
    ...
    with respx.mock:
        respx.get("https://api.github.com/repos/org/repo/pulls/1/reviews").respond(json={"error": "failed"})
        async with forge as client:
            pr = await client.get_pr_reviews(pr)
...
async def test_create_pr(forge_info, mocker):
    ...
    with respx.mock:
        respx.post("https://api.github.com/repos/org/repo/pulls").respond(
            json={"number": 2, "title": "Fix", "head": {"ref": "feat", "sha": "111"}, "draft": False}
        )
        async with forge as client:
...
async def test_get_pr_diff(forge_info, mocker):
    ...
    with respx.mock:
        respx.get("https://api.github.com/repos/org/repo/pulls/1").respond(text="diff --git a/b b/b")
        async with forge as client:
