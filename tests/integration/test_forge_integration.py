...
async def test_github_forge_list_prs_integration(mocker) -> None:
    ...
    mock_resp1.json.return_value = [
        {"number": 1, "title": "PR 1", "user": {"login": "user1"}, 
         "head": {"ref": "feat/1", "sha": "abc"}, "html_url": "url1",
         "updated_at": "2026-04-10T10:00:00Z"}
    ]
    mock_resp1.headers = {"link": '<https://api.github.com/repos/org/repo/pulls?page=2>; rel="next"'}
    mock_resp1.raise_for_status = MagicMock()

    mock_resp2 = MagicMock()
    mock_resp2.json.return_value = [
        {"number": 2, "title": "PR 2", "user": {"login": "user2"}, 
         "head": {"ref": "feat/2", "sha": "def"}, "html_url": "url2",
...
async def test_github_forge_get_ci_integration(mocker) -> None:
    ...
    mock_resp_status.json.return_value = {
        "statuses": [
            {"context": "security/audit", "state": "success"}
        ]
    }
    ...
    pr = ForgePR(
        number=1, title="t", author="a", branch="b", head_sha="abc",
        is_draft=False, url="", updated_at=datetime.now(UTC)
    )
