...
async def test_scan_repo_integration(mocker) -> None:
    ...
    # 1. Mock GitClient
    mock_git = MagicMock()
    mock_git.get_remote_url = AsyncMock(return_value="https://github.com/org/repo.git")
    mocker.patch("radioactive_ralph.pr_manager.GitClient", return_value=mock_git)
    
    # 2. Mock ForgeClient
    from radioactive_ralph.forge.base import ForgePR, ForgeCI, CIState, ForgeInfo
    ...
    f_pr = ForgePR(
        number=10, title="Fix bug", author="alice", branch="bugfix",
        head_sha="sha1", is_draft=False, url="http://gh/10",
        updated_at=datetime.now(UTC)
    )
    mock_forge.list_prs = AsyncMock(return_value=[f_pr])
    ...
    mock_forge.get_pr_reviews = AsyncMock(return_value=f_pr_approved)
    
    # Mock get_forge_client to return our mock_forge
    # It's an async context manager, so we need to mock __aenter__
    mock_forge.__aenter__ = AsyncMock(return_value=mock_forge)
    mock_forge.__aexit__ = AsyncMock(return_value=None)
